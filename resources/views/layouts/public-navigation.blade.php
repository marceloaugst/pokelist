<nav x-data="{ open: false }" class="border-b border-slate-600 bg-gradient-to-r from-slate-700 to-slate-800 shadow-xl">
    ...
    <div class="flex h-16 justify-between">
      <div class="flex">
        ...
        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
          <a href="{{ route('pokedex.index') }}"
            class="{{ request()->routeIs('pokedex.*') || request()->routeIs('home') ? 'border-yellow-400 text-yellow-400' : 'border-transparent text-slate-300 hover:text-yellow-400 hover:border-yellow-400/50' }} inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium leading-5 transition duration-150 ease-in-out">
            {{ __('Pokédx') }}
          </a>
          @auth
      ...
      <div class="hidden sm:ms-6 sm:flex sm:items-center">
        ...
        <div class="flex items-center space-x-4">
          <a href="{{ route('login') }}"
            class="rounded-lg border border-yellow-500 bg-transparent px-4 py-2 text-sm font-semibold text-yellow-400 transition-all duration-300 hover:bg-yellow-500 hover:text-slate-900">
            {{ __('Login') }}
          </a>
          <a href="{{ route('register') }}"
            class="rounded-lg bg-gradient-to-r from-yellow-500 to-orange-500 px-4 py-2 text-sm font-semibold text-slate-900 shadow-lg shadow-yellow-500/30 transition-all duration-300 hover:scale-105 hover:from-yellow-600 hover:to-orange-600">
            {{ __('Registrar') }}
          </a>
        </div>
        @endauth
  ...
  <div :class="{ 'block': open, 'hidden': !open }" class="hidden bg-slate-800 sm:hidden">
    <div class="space-y-1 pb-3 pt-2">
      <a href="{{ route('pokedex.index') }}"
        class="{{ request()->routeIs('pokedex.*') || request()->routeIs('home') ? 'border-yellow-400 text-yellow-400 bg-slate-700' : 'border-transparent text-slate-300 hover:text-yellow-400 hover:bg-slate-700 hover:border-yellow-400' }} block w-full border-l-4 py-2 pe-4 ps-3 text-start text-base font-medium transition duration-150 ease-in-out">
        {{ __('Pokédx') }}
      </a>
      @auth
      <a href="{{ route('pokemons.index') }}"
    ...
    <div class="border-t border-slate-700 pb-4 pt-4">
      <div class="space-y-2 px-4">
        <a href="{{ route('login') }}"
          class="block w-full rounded-lg border border-yellow-500 bg-transparent py-2 text-center text-sm font-semibold text-yellow-400 transition-all duration-300 hover:bg-yellow-500 hover:text-slate-900">
          {{ __('Login') }}
        </a>
        <a href="{{ route('register') }}"
          class="block w-full rounded-lg bg-gradient-to-r from-yellow-500 to-orange-500 py-2 text-center text-sm font-semibold text-slate-900 shadow-lg shadow-yellow-500/30 transition-all duration-300">
          {{ __('Registrar') }}
        </a>
      </div>
    </div>
