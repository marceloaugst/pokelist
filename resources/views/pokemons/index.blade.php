                        ...
                        <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            @foreach ($pokemons as $pokemon)
                                <div class="pokedex-card glow-card group relative p-6">
                                    ...
                                    <!-- Mini Chart Preview -->
                                    <div class="mb-6">
                                        <h4 class="mb-3 text-center text-sm font-semibold text-slate-300">Visão
                                            Geral de Status</h4>
                                        <div class="relative mx-auto" style="width: 120px; height: 120px;">
                                            <svg width="120" height="120" viewBox="0 0 120 120"
                                                ...
                                                <line x1="60" y1="60" x2="89" y2="37"
                                                    stroke="#64748b" stroke-width="1" />
                                                <line x1="60" y1="60" x2="78" y2="93"
                                                    stroke="#64748b" stroke-width="1" />
                                                <line x1="60" y1="60" x2="42" y2="93"
                                                    stroke="#64748b" stroke-width="1" />
                                                <line x1="60" y1="60" x2="31" y2="37"
                                                    stroke="#64748b" stroke-width="1" />
                                                ...
                                                @php
                                                    ...
                                                    $statsForChart = [
                                                        'speed' => $pokemon->game_speed,
                                                        'attack' => $pokemon->game_attack,
                                                        'sp_attack' => $pokemon->game_sp_attack,
                                                        'defense' => $pokemon->game_defense,
                                                        'hp' => $pokemon->game_hp,
                                                    ];

                                                    $gamePoints = [];
                                                    foreach (
                                                        ['speed', 'attack', 'sp_attack', 'defense', 'hp']
                                                        as $index => $statName
                                                    ) {
                                                        $angle = $startAngle + $index * $angleStep;
                                                        $gameValue = $statsForChart[$statName] ?? 50;
                                                        $gameScale = min($gameValue / $maxStat, 1);
                                                        $gameX = $center['x'] + $radius * $gameScale * cos($angle);
                                                        $gameY = $center['y'] + $radius * $gameScale * sin($angle);
                                                        $gamePoints[] = round($gameX, 1) . ',' . round($gameY, 1);
                                                    }
                                                    $gamePointsStr = implode(' ', $gamePoints);
                                                @endphp

                                                <!-- Game stats polygon -->
                                                <polygon points="{{ $gamePointsStr }}" fill="rgba(59, 130, 246, 0.3)"
                                    ...
                                    <div class="mb-6">
                                        <div
                                            ...
                                            <div class="mb-3 flex items-center justify-between">
                                                ...
                                                <span
                                                    class="text-{{ $overallColor }}-400 text-sm font-bold">{{ $overallStatus }}</span>
                                            </div>
                                            <div class="stat-bar">
                                        ...
                                        @if ($pokemon->notes)
                                            <div
                                                class="mt-4 rounded-lg border-l-4 border-yellow-400 bg-yellow-500/10 p-3">
                                                <span
                                                    class="text-xs italic text-yellow-300">{{ Str::limit($pokemon->notes, 50) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex gap-3">
                                        ...
                                        <form action="{{ route('pokemons.destroy', $pokemon) }}" method="POST"
                                            ...
                                            <button type="submit"
                                                ...
                                                class="w-full rounded-xl bg-red-500 py-3 text-sm font-bold text-white transition-colors hover:bg-red-600">
                                                Remover
                                            </button>
                                        </form>
                                    </div>
